{ pkgs, ... }:
{
        ...
        //        ),
         ...
         //       ),
                (
                    ...
                    pane: Split(
                        direction: Horizontal,
                        panes: [(size: "40%", pane: Pane(AlbumArt)), (size: "100%", borders: "ALL", pane: Pane(Lyrics), vertical_align: Bottom)],
                    ),	
                ),
            ],
        )
  ...
  # Menyertakan folder utils
  xdg.configFile."rmpc/utils".source = ./utils;

  # Menyertakan file script individual dan memastikannya executable
  xdg.configFile."rmpc/increment_play_count" = {
    source = ./increment_play_count;
    executable = true; # Sangat penting untuk script!
  };

  xdg.configFile."rmpc/inspect_log.sh" = {
    source = ./inspect_log.sh;
    executable = true;
  };
}
